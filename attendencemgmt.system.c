...
void disp()
{
 ...
 printf("  %d\t\t%s\t%s\n", stud.rollno, stud.name, stud.course);
 fclose(fp1);
}
 ...
 else
 {
  ...
  while (fread(&stud, sizeof(stud), 1, fpo))
  {
   ...
   else
   {
    ...
    printf("\n\t2. Update course of Roll Number %d", r);
    printf("\n\t3. Update both Name and course of Roll Number %d", r);
    printf("\nEnter your choice:");
    scanf("%d", &ch);
    switch (ch)
    {
...
void sort()
{
 ...
 for (i = 0; i<count; i++)
 {
  ...
  {
   if (a[i] == stud.rollno)
    printf("\n %d\t\t %s \t\t %s",stud.rollno, stud.name, stud.course);
  }
